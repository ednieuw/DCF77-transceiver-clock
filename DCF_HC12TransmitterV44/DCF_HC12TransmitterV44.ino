...
const byte MenuItems = 16;                  // No of entries in menu
...
//--------------------------------------------
                    #ifdef DCFTINY
const byte MSECINT  = 10;
const byte DCFREADS = 25;
uint32_t DCFmsTick;                       // the number of millisecond ticks since we last incremented the second counter
byte     EdMin              = 0;
byte     ThMin              = 0;
byte     EdThMin            = 0;          // valid times per minute counters for DCFtiny and DCF77
bool     DCFEd              = false;      // 
...
uint32_t ValidTimes         =  0;         // Counter of recorded valid times
...
struct EEPROMstorage {                    // 243 bytes long
  byte LightReducer;
...
void Reset(void)
{
 ...
 mSecInterval         = MSECINT;                   // if DCF_ReadFulltime = false this is time in msec between readings
 DCFreadsInLoop       = DCFREADS;                  // and this are the number of reading in a loop
 WriteLightReducer(LightReducer);                  // Default intensity for this clock
 StoreLowerBrightness(LowerBrightness);            // Default Lower Brightness for this clock
 WritemSecInterval(mSecInterval);                  // DCFtiny interval loop in EEPROM
 WriteDCFreadsInLoop(DCFreadsInLoop);              // DCFtiny interval loop in EEPROM
 ...
 if( millis() - msTick > 999)                                               // Flash the onboard Pin 13 Led so we know something is happening
   {    
    ...
    GetTijd(0);                                                             // synchronize time with RTC clock    
                           #if defined LEDsInstalled || defined LEDs24RingInstalled               
    DimLeds(false);                                                         // every second an intensitiy check and update from LDR reading  
 ...
 if(HC12transmit)                                                           // if HC12 is allowed to transmit
  {
   switch(Isecond)                                                          // When HC12 sends it disturbs DCF-signal. In seconds 1 - 19 are whether info bits
    {
   case  2: sprintf(sptext,"@%s|%0.2d:%0.2d:%0.2d ",STATION,Inow.hour(),Inow.minute(),Inow.second());
...
//--------------------------------------------
 ...
 { 
  ...
  if(MinutesSinceStart> 0X0FFFFFFF)           // Reset counters before overflow
     {Ed = Thijs = EdThijsGelijk = ValidTimes = MinutesSinceStart = 0;  }
  TotalEff =  constrain((100*ValidTimes/(MinutesSinceStart+1)),1,99);  
  DCF_signal--; 
  ...
  analogWrite(DCFgood, DCF_signal);           // The LED intensity displays the signal quality
                           #ifdef MAX7219info                     
  lc.shutdown(0,false); 
  lc.clearDisplay(0);                         // and clear the display 1
  lc.shutdown(1,false); 
  lc.clearDisplay(1);                         // and clear the display 2
                           #endif MAX7219info     
...
//--------------------------------------------
 void EveryHourUpdate(void)
 {
  ...
  Mem.HourEff[0][lasthour] = EdMin;        // Enter the noof valid minutes for the last hour for DCFtiny
  Mem.HourEff[1][lasthour] = ThMin;        // Enter the noof valid minutes for the last hour for DCF77
  Mem.HourEff[2][lasthour] = EdThMin;      // Enter the noof valid minutes for the last hour for Both together  

  sprintf(sptext,"Mem Tot DCFtiny:%0.2d DCF77:%0.2d Both%0.2d",EdMin, ThMin, EdThMin);     Tekstprintln(sptext);
  sprintf(sptext,"MemHour DCFtiny:%0.2d DCF77:%0.2d Both%0.2d",Mem.HourEff[0][lasthour], 
                 Mem.HourEff[1][lasthour], Mem.HourEff[2][lasthour]);     Tekstprintln(sptext);
  EdMin = ThMin = EdThMin = 0;             // Reset the minute counters
...
//--------------------------------------------
 void EveryDayUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                          // Calculate average effectivity 
    numerator = denominator = 0;
    for(n=0;n<24;n++) {if(Mem.HourEff[i][n]) { numerator += Mem.HourEff[i][n]; denominator++;} }
    Mem.DayEff[i][lastday] = (100 * numerator) / (denominator*60);    // Mem.HourEff contains the valid time count. Now we store only the percentage
   }
                           #ifdef MAX7219info                     
   lc.shutdown(0,false);                       // Reset the display completely
   lc.clearDisplay(0);                         // Clear the display 1
   lc.shutdown(1,false); 
   lc.clearDisplay(1);                         // Clear the display 2
                           #endif MAX7219info   
  sprintf(sptext,"MemDay DCFtiny:%0.2d DCF77:%0.2d Both%0.2d",Mem.DayEff[0][lastday], 
          Mem.DayEff[1][lastday], Mem.DayEff[2][lastday]);     Tekstprintln(sptext); 
  lastday = constrain(Iday,1,31);
  if (Imonth != lastmonth) EveryMonthUpdate();
  EEPROM.put(0,Mem);                          // Store the statistics data in EEPROM
 }
...
//--------------------------------------------
 void EveryMonthUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                          // Calculate average effectivity 
    numerator = denominator = 0;
    for(n=0;n<32;n++) {if(Mem.DayEff[i][n]) { numerator += Mem.DayEff[i][n]; denominator++;} }
    Mem.MonthEff[i][lastmonth] = numerator / denominator;
   }
  sprintf(sptext,"MemMonth DCFtiny:%0.2d DCF77:%0.2d Both%0.2d", Mem.MonthEff[0][lastmonth], 
              Mem.MonthEff[1][lastmonth], Mem.MonthEff[2][lastmonth]);     Tekstprintln(sptext); 
...
//--------------------------------------------
 void EveryYearUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                          // Calculate average effectivity 
    numerator = denominator = 0;
    for(n=0;n<10;n++) {if(Mem.MonthEff[i][n]) { numerator += Mem.MonthEff[i][n]; denominator++;} }
    Mem.YearEff[i][lastyear] = numerator / denominator;
   }
  sprintf(sptext,"MemYear DCFtiny:%0.2d DCF77:%0.2d Both%0.2d",Mem.YearEff[0][lastyear], 
           Mem.YearEff[1][lastyear], Mem.YearEff[2][lastyear]);     Tekstprintln(sptext);   
  lastyear = Iyear%10;
...
//   setTime(DCFtime);
   ...
   DateTime DCFtijd (tm.Year+1970, tm.Month, tm.Day, tm.Hour, tm.Minute, 0);   // Convert the time in the time_t DCFtime to Datetime so we can use .unixtime 
...
//       RTCklok.adjust(DCFtime); 
   ...
   Thijs++;
   ThMin++;
   DCFThijs = true;                            // Used to check if DCF77 and DEtiny receive both a good time    
  }
                             #endif DCFMOD                             
                             #ifdef DCFTINY 
     if(DCF_ReadFulltime)                           // if all pulses are measured
   {
  ...
  if((millis() - DCFmsTick) >999) //= 1000)  // Compute every second the received DCF-bit to a time 
   { 
 //   lastsecond = Inow.second(); //Isecond;
    while (1-digitalRead(DCF_PIN));              // avoid splitting a positve signal add evt (delay(10) and un every hour same while with delay(500)
    DCFmsTick = millis();   //wait until pulse = 0
      ...
      if(OKstatus == 1)                         // If time flag was OK and date flag also OK  (OKstatus == 1)
        {
         ...
         int TimeDiff = (int)(abs(now.unixtime() - DCFtijd.unixtime()));
//         sprintf(sptext,"RTC is %d sec different from DCFtiny time! RTC:%0.2d:%0.2d:%0.2d DCF:%0.2d:%0.2d:%0.2d ",
//                                TimeDiff, Inow.hour(), Inow.minute(),Inow.second(),Dhour, Dminute,0); 
//         Tekstprintln(sptext);  
         if(TimeDiff > 2  && DCFlocked)       // > in Seconds. If difference between DCF and RTC time update the RTC
           {
            sprintf(sptext,"RTC is %d sec different from DCFtiny time! Updating. RTC:%0.2d:%0.2d:%0.2d DCF:%0.2d:%0.2d:%0.2d ",
                               TimeDiff,Inow.hour(), Inow.minute(),Inow.second(),Dhour,Dminute,0); 
            Tekstprintln(sptext);   
...
byte UpdateDCFclock(void)
{
 ...
 tmElements_t Prd;                                                             // predicted time

 if (Bitpos >= 60) 
     {
      Bitpos = 0;
      MinOK = HourOK = YearOK = 2;
      if (DCFEd && DCFThijs) { EdThijsGelijk++; EdThMin++; DCFlocked=true;}     // Count the times both DCFalgorithm received a good time. Lock DCFtime
      if (DCFEd || DCFThijs) { ValidTimes++;  DCF_signal+=2;}                   // Add 2 to DCF_signal because one is subtracted here after
         DCF_signal = constrain( DCF_signal,1,99);
      DCFEd = DCFThijs = false;                                                 // turn off good DCF-times received
                           #ifdef LEDsInstalled
      LedsOffOuterring();
  ...
  {                                                                             // It is not binary but "Binary-coded decimal". Blocks are checked with an even parity bit. 
   case   0: BMinutemark = Receivebit;
             if( MinOK && HourOK && YearOK) 
               { 
                TimeOK = 1;        
                if(DCFlocked && abs((((Dyear*12+Dmonth)*31)+Dday)-((((Iyear-2000)*12+Imonth)*31)+Iday)) >1)
                  {
//                   sprintf(sptext,"Date is different from DCFtiny date:%0.2d-%0.2d-%0.2d RTC:%0.2d:%0.2d:%0.2d ",
//                               Dday, Dmonth,Dyear ,Iday,Imonth,Iyear);      Tekstprintln(sptext);
                   TimeOK = 0;                                                  // if a few DCF signal are received valid time control will be tighter 
                  }
               if(DCFlocked && abs((Dhour*60+Dminute)-(Ihour*60+Iminute)) >2)
                 {
//                  sprintf(sptext,"DCFtiny time is different from DCFtiny time:%0.2d:%0.2d:%0.2d RTC:%0.2d:%0.2d:%0.2d ",
//                               Dhour,Dminute,0, Ihour, Iminute,Isecond);     Tekstprintln(sptext); 
                 TimeOK = 0;     
                 }
               }
             if( MinOK && HourOK && !YearOK) 
               { 
                if (abs((Dhour*60+Dminute)-(Ihour*60+Iminute))<2 && DCFlocked) {TimeOK = 2;} 
               }
  ...
  if(PrintDebugInfo)
    {
     ...
     sprintf(sptext,"%5ld %5ld %2ld%% %2ds (%d) %0.2d:%0.2d %0.2d-%0.2d-%0.4d/%0.1d F%d",
                     ...
                     Receivebitinfo, Dhour, Dminute, Dday, Dmonth, 2000+Dyear, Dwday, SignalFaults); Tekstprint(sptext); 
     sprintf(sptext," Ed:%ld Th:%ld Min:%ld OK:%d",Ed, Thijs, MinutesSinceStart,TimeOK); Tekstprintln(sptext); 
    }
                            #ifdef MAX7219info
 ...
 else sprintf(sptext,"%0.3d_%0.2d%0.2d%0.2d%0.2d%0.2d%0.2d",(int)(1000*SumSecondSignal/SumSignalCounts),Dhour, Dminute,Bitpos,DCF_signal,TotalEff, (int)(100*Ed/MinutesSinceStart));
...
{
                              #if defined DCFMOD || defined DCFTINY 
 sprintf(sptext," Ed:%ld Th:%ld Both:%ld Valid:%ld Tot:%ld ",Ed, Thijs, EdThijsGelijk,ValidTimes, MinutesSinceStart);
 Tekstprint(sptext);
 sprintf(sptext," DCF Eff:%0.2d%% Signal:%0.2d %s",TotalEff,DCF_signal, DCFlocked ? "DCF" : "RTC");
 Tekstprintln(sptext); 
 int i,n;
// hour
 ...
 for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                         
    for(n=0;n<24;n++) { sprintf(sptext,"%0.2d ",Mem.HourEff[i][n]); Tekstprint(sptext);}
        Tekstprintln(""); 
    }
...
//  Write to EEPROM
//--------------------------------------------
void xEEPROMwrite(byte pos, byte waarde)
{ 
...
void ReworkInputString(String InputString)
{
 ...
 if ( InputString[0] > 64)                                           // Does the string start with a letter?
  ...
  switch (val)
   {
    ...
    case 'c':
             ...
             for(i=0;i<3;i++) {for(n=0;n<24;n++) { Mem.HourEff[i][n] = 0; } }
  ...
  else Tekstprintln("**** Length fault. Enter hhmmss ****");              
