...
const byte MenuItems = 14;                  // No of entries in menu
...
bool     DCFThijs           = false;      // Flag to detect if DCF77 library received a good time
...
uint32_t EdOrTh             =  0;         // Count for valid retrieved times for DCFtiny and DCF77
...
struct EEPROMstorage {                    // 246 bytes long
  byte LightReducer;
...
//Tijd_display.showNumberDec(displaytime, true);                             // Prints displaytime without center colon.  
  ...
  if(MinutesSinceStart> 0X0FFFFFFF)                                          // Reset counters before overflow
     { Ed = Thijs = EdOrTh = ValidTimes = MinutesSinceStart = 0; }
  TotalEff =  constrain((100*ValidTimes/(MinutesSinceStart)),1,99);  
  DCF_signal--; 
...
//--------------------------------------------
 void EveryHourUpdate(void)
 {
  ...
  Mem.HourEff[2][lasthour] = constrain(SignalFaults,0,255); // Enter the noof SignalFaults 
  SignalFaults = 0;
  DCFlocked = false;                                        // It is time to receive a DCF-time code  
//  sprintf(sptext,"Mem Tot DCFtiny:%0.2d DCF77:%0.2d SignalFaults:%0.2d",EdMin, ThMin, SignalFaults);    Tekstprintln(sptext);
  sprintf(sptext,"MemHour DCFtiny:%0.2d DCF77:%0.2d Faults: %d",
                  Mem.HourEff[0][lasthour], Mem.HourEff[1][lasthour], Mem.HourEff[2][lasthour]); Tekstprintln(sptext);
 ...
 void EveryDayUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                             // Calculate average effectivity 
    ...
    for(n=0;n<24;n++) {if(Mem.HourEff[i][n]>0) { numerator += Mem.HourEff[i][n]; denominator++;} }
    Mem.DayEff[i][lastday] = numerator / denominator;
   }
...
//--------------------------------------------
 void EveryMonthUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                          // Calculate average effectivity 
    numerator = denominator = 0;
    for(n=0;n<32;n++) {if(Mem.DayEff[i][n]>0) { numerator += Mem.DayEff[i][n]; denominator++;} }
    Mem.MonthEff[i][lastmonth] = numerator / denominator;
   }
  sprintf(sptext,"MemMonth DCFtiny:%0.2d DCF77:%0.2d SignalFaults: %d", Mem.MonthEff[0][lastmonth], 
...
//--------------------------------------------
 void EveryYearUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                          // Calculate average effectivity 
    numerator = denominator = 0;
    for(n=0;n<10;n++) {if(Mem.MonthEff[i][n]>0) { numerator += Mem.MonthEff[i][n]; denominator++;} }
    Mem.YearEff[i][lastyear] = numerator / denominator;
   }
  sprintf(sptext,"MemYear DCFtiny:%0.2d DCF77:%0.2d SignalFaults: %d",Mem.YearEff[0][lastyear], 
...
void DCF77Check(void)
{
 ...
 if(DCF_ReadFulltime)                           // if all pulses are measured
  ...
  if((millis() - DCFmsTick) >995) //= 1000)     // Compute every second the received DCF-bit to a time 
   { 
    while (1-digitalRead(DCF_PIN))
       {
        SumSignalCounts++;                      // Avoid splitting a positve signal 
        SumSecondSignal++;
       }
 ...
 if (Bitpos >= 60) 
     {
      ...
      if (DCFEd && DCFThijs) { EdThMin++; DCFlocked=true;}                    // Count the times both DCFalgorithm received a good time. Lock DCFtime
      if (DCFEd || DCFThijs) { EdOrTh++; ValidTimes++;  DCF_signal+=2;}                  // Add 2 to DCF_signal because one is subtracted here after
      DCF_signal = constrain( DCF_signal,1,99);
  ...
  {                                                                            // It is not binary but "Binary-coded decimal". Blocks are checked with an even parity bit. 
   case   0: BMinutemark = Receivebit;
             if( MinOK && HourOK && YearOK) 
               { 
                TimeOK = 1;        
                if(DCF_signal>60 && abs((((Dyear*12+Dmonth)*31)+Dday)-((((Iyear-2000)*12+Imonth)*31)+Iday)) >1)   
                  {
...
//                               Dday, Dmonth,Dyear ,Iday,Imonth,Iyear);      Tekstprintln(sptext);
               ...
               if(DCF_signal>60 && abs((Dhour*60+Dminute)-(Ihour*60+Iminute)) >2)    
                 {
...
//                               Dhour,Dminute,0, Ihour, Iminute,Isecond);     Tekstprintln(sptext); 
             ...
             if(Dyear <20|| Dmonth==0 || Dmonth>12 || Dhour>23 || Dminute>59) TimeOK = 0;    // If time is definitely wrong return Time is not OK 
                             #ifdef LEDsInstalled 
             ColorLed(Minutemark,Receivebit ? orange: green);
...
//     sprintf(sptext," %0.3d:%0.3d :%+0.2d",msec, Interruptmsec[0],msec-Interruptmsec[0]); Tekstprintln(sptext);
 ...
 switch (Bitpos)                                                                // Decode the 59 bits to a time and date.
  {                                                                             // It is not binary but "Binary-coded decimal". Blocks are checked with an even parity bit. 
   case 0 ... 19: sprintf(sptext,"%0.3d %0.2d%0.2d",(int)(1000*SumSecondSignal/SumSignalCounts), TotalEff, SignalFaults); break;
        default:  sprintf(sptext,"%0.3d %0.2d%0.2d",(int)(1000*SumSecondSignal/SumSignalCounts),Dhour, Dminute);
  }
 }
 else sprintf(sptext,"%0.3d_%0.2d%0.2d%0.2d%0.2d%0.2d%0.2d",(int)(1000*SumSecondSignal/SumSignalCounts),Dhour, Dminute,Bitpos,DCF_signal,TotalEff, SignalFaults);
...
{
                              #if defined DCFMOD || defined DCFTINY 
// sprintf(sptext," Ed:%ld Th:%ld Both:%ld Valid:%ld Tot:%ld ",Ed, Thijs, EdThGelijk,ValidTimes, MinutesSinceStart);
// Tekstprint(sptext);
// sprintf(sptext," DCF Eff:%0.2d%% Signal:%0.2d %s",TotalEff,DCF_signal, DCFlocked ? "DCF" : "RTC");
// Tekstprintln(sptext); 
 int i,n;
...
//--------------------------------------------
void ColorMiddleRing(byte Fhour ,byte Nooffaults)
{
 for (int n=0; n<24;n++) 
  {  
   switch(Mem.HourEff[2][n])
   {
     ...
     case 121 ... 0xFE:   ColorLed(60 + n,  red   ); break; 
     case 0xFF:           ColorLeds("", 60, 95, purple);        // Reset and color the middle ring purple
     default:                                            break; 
    }
  }
 if (Fhour==23) Fhour = 0;
...
{ 
 strip.fill(BrightnessCalcFromLDR , 0, 60);    // Color the outer seconds info ring LEDs faint blue
}
...
{                                                                                          
 if (SecPulse)   // if a second has passed 
 {
  ...
  MaxPhotocell = MaxPhotocell < LDR_read ? LDR_read : MaxPhotocell;        // sprintf(sptext,"LDR read: %d MinPhotocell %d\n",LDR_read, MinPhotocell);   Tekstprint(sptext);
             ...
             if(InputString.length() == 1)
               {
                ...
                PrintStats();                                            // Print statistics  
                                                                  ...
                                                                  break;
                Tekstprintln("-Hier mag ik niet zijn <---- Statistics  -----");              
    case 'T':
    case 't':
