...
V50  Optimized time decoding from DCF   Identical to DCF77-NoIntV03 
...
//--------------------------------------------
 void EveryHourUpdate(void)
 {
  ...
  Mem.HourEff[2][lasthour] = constrain(SignalFaults,0,99);                    // Enter the noof SignalFaults last hour, >99 disrupt lay-out of statistics 
  DCFlocked = false;                                                           // It is time to receive a DCF-time code  
  sprintf(sptext,"MemHour DCFtiny:%0.2d DCF77:%0.2d Faults: %d",
    Mem.HourEff[0][lasthour], Mem.HourEff[1][lasthour], Mem.HourEff[2][lasthour]); 
  Tekstprintln(sptext);
  ...
  EdMin = ThMin = EdThMin = SignalFaults = 0;                                  // Reset the minute counters
 ...
 void EveryDayUpdate(void)
 {
  ...
  for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                                                           // Calculate average valid minutes cq Nooffaults/hour over one day 
    ...
    for(n=0;n<24;n++) {if(Mem.HourEff[i][n]>0) 
      { 
       numerator += Mem.HourEff[i][n]; 
       denominator++;} 
      }
  ...
  EEPROM.put(0,Mem);                                                          // Store the Mem. data in EEPROM
 }
  ...
  if(PrintDebugInfo)
    {
     ...
     sprintf(sptext," Ed:%ld Th:%ld Min:%ld OK:%d L:%s",Ed, Thijs, MinutesSinceStart,TimeOK,DCFlocked?"Y":"N"); Tekstprintln(sptext); 
...
{
                              #if defined DCFMOD || defined DCFTINY 
 sprintf(sptext," Ed:%ld Th:%ld Both:%ld Valid:%ld Tot:%ld ",Ed, Thijs, EdThMin,ValidTimes, MinutesSinceStart);
 Tekstprint(sptext);
 sprintf(sptext," DCF Eff:%0.2d%% Signal:%0.2d %s",TotalEff,DCF_signal, DCFlocked ? "DCF" : "RTC");
 Tekstprintln(sptext); 
 int i,n;
