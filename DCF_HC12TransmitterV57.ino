...
V56  Added counters for wrong times reported by both decoding methods DCF77 and DCFTint 
V57  Removed compiler warnings   Changed Mem.mSecInterval  !!Not checked 
...
//byte     EdMinhour          = 0;                          // Counter of valid times per hour for DCFtiny
//byte     ThMinhour          = 0;                          // Counter of valid times per hour for DCF77
...
uint32_t EdMin              =  0;                         // Counter of total valid tdate/imes since start for DCFtiny 
uint32_t ThMin              =  0;                         // Counter of totalvalid date/times since start for DCF77
uint32_t EdMinTijd          =  0;                         // Counter of valid times times since start for DCF77
uint32_t EdTh               =  0;                         // Valid times per hour counters for DCFtiny and DCF77
uint32_t ThWrong            =  0;                         // Different times reported DCFtiny and DCF77
uint32_t EdWrong            =  0;                         // Different times reported DCFtiny and DCF77
...
struct RAMstorage {                                         // 246 bytes long. Store the hour performance in RAM
  byte HourEff[3][24]= {0};
} RMem;
...
{                                        
 ...
 EdMin             = Mem.EdMin;                                              // Counter of total valid times since start for DCFtiny 
 ThMin             = Mem.ThMin;                                              // Counter of totalvalid times since start for DCF77
...
//--------------------------------------------
void Tekstprint(const char tekst[])
{
 Serial.print(tekst);    
                          #ifdef BLUETOOTHMOD   
                          #if defined(ARDUINO_SAMD_MKRWIFI1010) 
...
void Tekstprintln(const char tekst[])
{
 ...
 if(HC12transmit)                                                             // if HC12 is allowed to transmit
  {
   switch(Isecond)                                                            // When HC12 sends it disturbs DCF-signal. In seconds 1 - 19 are whether info bits
    {
   case  1: sprintf(sptext,"@%s|%02d:%02d:%02d ",STATION,Inow.hour(),Inow.minute(),Inow.second());
            if (DCFlocked)  HC12.print(sptext);                               // Print the time only if DCF time is trusted 
                            Tekstprint(sptext);     
            sprintf(sptext,"%02d-%02d-%04d|",Inow.day(),Inow.month(),Inow.year());
            if (DCFlocked)  HC12.print(sptext);                               // Print the time only if DCF time is trusted
                            Tekstprint(sptext);
            sprintf(sptext,"DCF-eff:%02d%% Signal:%02d |%s",TotalEff,DCF_signal, DCFlocked ? "DCF" : "RTC");
            if (DCFlocked)  HC12.println(sptext);                             // Print the time only if DCF time is trusted
                            Tekstprintln(sptext); 
                                                               break;  
   case  2: sprintf(sptext,"T%02d%02d%02d ",Ihour,Iminute,Isecond);        // Send the time as Thhmmss
   ...
   case  5: if(Iminute == 0)                                                  // Send date once an hour as Dddmmyyyy
              {    
               sprintf(sptext,"D%02d%02d%04d",Iday,Imonth,Iyear);
...
// if (Inow.hour() < 10) Tijd_displayIC2.writeDigitNum(0, 0);  // also pad the first hour digit with zero
 ...
 if(MinutesSinceStart> 0X0FFFFFF0)                                            // Reset counters before overflow
    { EdMin = ThMin = EdTh = EdWrong = ThWrong = ValidTimes = MinutesSinceStart = 0; }
...
void EveryHourUpdate(void)
{
 ...
 RMem.HourEff[2][lasthour] = (byte)constrain((int)SignalFaults,0,99);              // Enter the noof SignalFaults last hour, >99 disrupt lay-out of statistics 
 DCFlocked = false;                                                            // It is time to receive a DCF-time code  
 sprintf(sptext,"MemHour DCFtiny:%02d DCF77:%02d Faults: %d",
...
void EveryDayUpdate(void)
{
 ...
 for(i=0;i<3;i++) {for(n=lastday+1;n<32;n++) {Mem.DayEff[i][n] = 0;}}        // Set following days of the month to zero
  sprintf(sptext,"MemDay DCFtiny:%02d DCF77:%02d SignalFaults: %d",Mem.DayEff[0][lastday], 
         Mem.DayEff[1][lastday], Mem.DayEff[2][lastday]);     
 Tekstprintln(sptext); 
 Mem.EdMin             = EdMin;                                              // Counter of total valid times since start for DCFtiny 
 Mem.ThMin             = ThMin;                                              // Counter of totalvalid times since start for DCF77
 Mem.EdTh              = EdTh;                                               // Valid times per hour counters for DCFtiny and DCF77
 Mem.EdWrong           = EdWrong;                                            // Different times reported DCFtiny and DCF77
 Mem.ThWrong           = ThWrong;                                            // Different times reported DCFtiny and DCF77
...
//--------------------------------------------
 void EveryMonthUpdate(void)
 {
  ...
  sprintf(sptext,"MemMonth DCFtiny:%02d DCF77:%02d SignalFaults: %d", 
...
//--------------------------------------------
 void EveryYearUpdate(void)
 {
  ...
  sprintf(sptext,"MemYear DCFtiny:%02d DCF77:%02d SignalFaults: %d",
...
//   sprintf(sptext,"RTC is %d sec different from DCF77 time! RTC:%02d:%02d:%02d DCF:%02d:%02d:%02d ",
...
//   Tekstprintln(sptext); 
   if(TimeDiff > 2  && DCFlocked)                                             // > in Seconds. If difference between DCF and RTC time update the RTC
      {
        sprintf(sptext,"RTC differs %ld sec from DCF77. RTC:%02d:%02d:%02d DCF:%02d:%02d:%02d ",
                        TimeDiff,Inow.hour(), Inow.minute(),Inow.second(), tm.Hour,tm.Minute, tm.Second); 
        Tekstprintln(sptext);       
   ...
   { 
    ...
    if(byte OKstatus = UpdateDCFclock())                                      // If after 60 sec a valid time is calculated, sent it to the HC-12 module
      ...
      if(OKstatus == 2)                                                       // If time flag was OK and date flag was NOK
        {
        sprintf(sptext,"       TIME OK --> %02d:%02d  ",Dhour, Dminute); Tekstprintln(sptext);           
    //  RTCklok.adjust(DateTime(Inow.year(), Inow.month(), Inow.day(), Dhour, Dminute, 0));
         EdMin++;                                                             // Count the valid times per hour
         EdMinTijd++;                                                         // Count the valid times since start
        }
      if(OKstatus == 1)                                                       // If time flag was OK and date flag also OK  (OKstatus == 1)
        {
         sprintf(sptext,"TIME & DATE OK --> %02d:%02d %02d-%02d-%04d ",Dhour, Dminute, Dday, Dmonth, 2000+Dyear);
         Tekstprintln(sptext);
         ...
         sprintf(sptext,     "%02d:%02d %02d-%02d-%04d",Dhour, Dminute, Dday, Dmonth, 2000+Dyear );   
         sprintf(sptextRTC,  "%02d:%02d %02d-%02d-%04d",Inow.hour(),Inow.minute(),Inow.day(), Inow.month(), Inow.year());            
         if(DCFTh && strcmp(sptext,sptextDCF77)!=0)               // if reported date/times are different
           {
            char fftext[90];
            sprintf(fftext,"Differences! -- \nRTC:%s\n Ed:%s\n Th:%s", sptextRTC, sptext, sptextDCF77); 
            Tekstprintln(fftext);
            if(strcmp(sptextRTC,sptextDCF77)!=0)  ThWrong++;
            if(strcmp(sptextRTC,sptext)!=0)       EdWrong++;
           }         
         DateTime now = RTCklok.now();
         ...
         if(TimeDiff > 2)                                                    // > in Seconds. If difference between DCF and RTC time update the RTC
           {
            sprintf(sptext,"RTC differs %ld sec from DCFtiny! Updating. RTC:%02d:%02d:%02d DCF:%02d:%02d:%02d ",
            ...
            if (MinutesSinceStart > 60) DCFlocked = true;                           // Every DCF time with OK status 1 or 2 are stored in the first 60 miutes
           }
         EdMin++;                                                             // Count the valid times per hour
  ...
  if(PrintDebugInfo)
    {
     ...
     if (PrintDebugInfoShort)
        {
         sprintf(sptext,"%5ld %5ld %2ld%%(%d)%02d:%02d:%02d %02d-%02d-%02d",
        ...
        {
         sprintf(sptext,"%5ld %5ld %2ld%% (%d) %02d:%02d:%02d %02d-%02d-%04d/%01d F%d",
                     SumSecondSignal, SumSignalCounts, 100*SumSecondSignal/SumSignalCounts,  
                     Receivebitinfo, Dhour, Dminute, Bitpos, Dday, Dmonth, 2000+Dyear, Dwday, SignalFaults); 
         Tekstprint(sptext); 
         sprintf(sptext," EdT:%ld, Ed:%ld Th:%ld EdW:%ld ThW:%ld Min:%ld OK:%d L:%s",EdMinTijd, EdMin, ThMin, EdWrong, ThWrong, MinutesSinceStart,TimeOK,DCFlocked?"Y":"N"); 
         Tekstprintln(sptext);         
        }
//     sprintf(sptext," %03d:%03d :%+0.2d",msec, Interruptmsec[0],msec-Interruptmsec[0]); Tekstprintln(sptext);
    }
                            #ifdef MAX7219info
 if(NUMBEROF7219DISPLAYS==1)
 {
  switch (Bitpos)                                                                // Decode the 59 bits to a time and date.
   {                                                                             // It is not binary but "Binary-coded decimal". Blocks are checked with an even parity bit. 
    case 0 ... 19: sprintf(sptext,"%03d_%02d%02d%02d_%02d_%02d",(int)
                   (1000*SumSecondSignal/SumSignalCounts),Iday,Imonth,Bitpos,TotalEff,SignalFaults);    break;
          default: sprintf(sptext,"%03d %02d%02d",(int)(1000*SumSecondSignal/SumSignalCounts),Dhour, Dminute);
   }
 }
 else 
  { 
   switch (Bitpos)                                                                // Print reading values in the displays.
    {
     case 0 ...  5: sprintf(sptext,"%08ld%08ld",ValidTimes, MinutesSinceStart);                        break;
     case        6: lc.clearDisplay(0);  lc.clearDisplay(1);
     case 7 ... 19: sprintf(sptext,"%03d_%02d%02d%02d_%02d_%02d",(int)
                    (1000*SumSecondSignal/SumSignalCounts),Iday,Imonth,Bitpos,TotalEff,SignalFaults);     break;
     case       20: lc.clearDisplay(1);
           default: sprintf(sptext,"%03d_%02d%02d%02d__%04d",(int)
...
{
                              #if defined DCFMOD || defined DCFTINY 
 sprintf(sptext,"DCFEd:%ld\nDCF77:%ld\nBoth:%ld\nValid times:%ld\nTotal minutes:%ld\n",EdMin, ThMin, EdTh, ValidTimes, MinutesSinceStart);
 Tekstprint(sptext);
 sprintf(sptext,"DCFEd Wrong:%ld\nDCF77 Wrong:%ld\n",EdWrong, ThWrong);
 Tekstprint(sptext); 
 sprintf(sptext,"DCF Eff:%02d%% Signal:%02d %s",TotalEff,DCF_signal, DCFlocked ? "DCF" : "RTC");
 Tekstprintln(sptext); 
 int i,n;
// hour
 ...
 for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                         
    for(n=0;n<24;n++) { sprintf(sptext,"%02d ",RMem.HourEff[i][n]); Tekstprint(sptext);}
        Tekstprintln(""); 
    }
...
//day
 ...
 for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                         
    for(n=1;n<32;n++) { sprintf(sptext,"%02d ",Mem.DayEff[i][n]); Tekstprint(sptext);}
    Tekstprintln(""); 
    }
...
//month
 ...
 for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                         
    for(n=1;n<13;n++) { sprintf(sptext,"%02d ",Mem.MonthEff[i][n]); Tekstprint(sptext);}
    Tekstprintln(""); 
    }
...
//year
 ...
 for(i=0;i<3;i++)  // i = 0,1,2 -> DCFtiny, DCF77,Both
   {                                         
    for(n=0;n<10;n++) { sprintf(sptext,"%02d ",Mem.YearEff[i][n]); Tekstprint(sptext);}
    Tekstprintln(""); 
    }    
...
//--------------------------------------------
void StoreLightReducer(int waarde)
{
...
//--------------------------------------------
void StoreLowerBrightness( int waarde)
{
...
//--------------------------------------------
void WritemSecInterval(int waarde){ Mem.mSecInterval = waarde; EEPROM.update(0,Mem.mSecInterval);}
  ...
  switch (val)
   {
    ...
    case 'C':
    case 'c':
             unsigned int i,n;         // i = 0,1,2 -> DCFtiny, DCF77,Both
             if(InputString.length() == 1)
               {
                ...
                Mem.EdMin = Mem.ThMin = Mem.EdTh = Mem.ValidTimes = Mem.MinutesSinceStart = 0;
                EdMin = ThMin = EdTh = ValidTimes = MinutesSinceStart = 0;             
